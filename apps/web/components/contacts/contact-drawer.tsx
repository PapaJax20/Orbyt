...
function CreateContactForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 pb-6">
      ...
      <button type="submit" disabled={create.isPending || !firstName.trim()}
...
function ViewContact({ contact, onClose }: { contact: ContactDetail; onClose: () => void }) {
  const utils = trpc.useUtils();
    ...
    <Drawer open={open} onClose={onClose} title={title}>
      {contactId ? (
        isLoading ? (
          ...
          <ViewContact contact={contact} onClose={onClose} />
        ) : (
